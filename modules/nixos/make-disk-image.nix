/*
  Modified https://github.com/NixOS/nixpkgs/blob/7cf092925906d588daabc696d663c100f2bbacc6/nixos/lib/make-disk-image.nix
  Adds partitionTableType "legacy+boot" to support Raspberry Pi
*/
...
This partition table type uses MBR.

- creates an FAT32 BOOT partition from 8MiB to specified `bootSize` parameter (256MiB by default);
- creates an primary ext4 partition starting after the boot partition and extending to the full disk image
...
assert (lib.assertOneOf "partitionTableType" partitionTableType [ "legacy+boot" "legacy" "legacy+gpt" "efi" "efixbootldr" "hybrid" "none" ]);
...
let format' = format; in let
  ...
  partitionDiskScript = { # switch-case
    ...
    "legacy+boot" = ''
      parted --script $diskImage -- \
        mklabel msdos \
        mkpart primary fat32 8MiB ${bootSize} \
        mkpart primary ext4 ${bootSize} -1
    '';
    none = "";
}.${partitionTableType};
  ...
  prepareImage = ''
    ...
    ${if diskSize == "auto" then ''
      ...
      '' else if partitionTableType == "legacy" || partitionTableType == "legacy+boot" then ''
